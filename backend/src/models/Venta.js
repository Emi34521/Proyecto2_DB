...
router.post("/", authMiddleware,
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Invoca fn_registrar_venta (función con lógica transaccional y excepciones)
      const [result] = await sequelize.query(
        `SELECT fn_registrar_venta(:cliente_id, :empleado_id, :productos::jsonb) AS venta_id`,
        {
          replacements: { cliente_id, empleado_id, productos: productosJson },
          transaction: t,
        }
      );

      await t.commit();
      const ventaId = result[0]?.venta_id;
